    ...
    public partial class SelectionFrm : Office2007Form
    {
        ...
        {            
            ...
            { 
                ...
                try
                {
                    Phicomm_WMS.OUTIO.tR_Tr_Sn.DelTrSn(trSn);
                }
                catch (Exception ex)
                {
                    Log.Error("DelTrSn ex: " + ex.Message);
                    ShowHint("删除TrSn信息异常: " + ex.Message, Color.Red);
                }

                //插入TrSn
                if (!InsertTrSn(textBox_stockout.Text.Trim(), comboBox_stockout.SelectedIndex, trSn))
                {                    
        ...
        public bool InsertTrSn(string woid, int stockNoType, string trsn)
        {
            ...
            {
                ShowHint("该工单类型为：" + stockNoType + ", 属于售出，委外，移库中的一种，不需要推送TrSn信息到线边仓", Color.Red);
                return true;
            }
            ...
            try
            {
                    ...
                    string.IsNullOrEmpty(dt1.Rows[0]["VENDER_NAME"].ToString().Trim()))
                {
                    ...
                    {
                        ShowHint("SearchFromRInventoryIdByKpNo异常:" + ex.Message, Color.Red);
                    }
                }
