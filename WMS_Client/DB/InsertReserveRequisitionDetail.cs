...
{
    public class InsertReserveRequisitonDetail : BaseDbUpdater
    {
        private List<string> _sqlList = new List<string>();

        public InsertReserveRequisitonDetail(DataTable dt)
            : base("insert function", DbName)
        {
            foreach (DataRow dr in dt.Rows)
            {
                string sql = "Insert Into r_reserve_requisition_detail (woid,material_no,qty,send_qty,PLANT,STORE_LOC,MOVE_TYPE,MOVE_PLANT,MOVE_STLOC,sub_type) Values('" +
